...
public class TaskController {
	...
	public String index(Model model) {
		...
		List<Task>openTasks=tasks.stream().filter(task->task.getStatus().equals("Open"))
				.collect(Collectors.toList());
		List<Task>inProgressTasks=tasks.stream().filter(task->task.getStatus().equals("In Progress"))
				.collect(Collectors.toList());
		List<Task>finishedTasks=tasks.stream().filter(task->task.getStatus().equals("Finished"))
				.collect(Collectors.toList());
	...
	public String editProcess(Model model, @PathVariable int id, TaskBindingModel taskBindingModel) {
		if (taskBindingModel.getStatus().equals("") || taskBindingModel.getTitle().equals("")) {
			...
			taskTemp.setId(id);
			taskTemp.setTitle(taskBindingModel.getTitle());
			taskTemp.setStatus(taskBindingModel.getStatus());
			model.addAttribute("task", taskTemp);
			model.addAttribute("view", "task/edit");
			return "base-layout";
